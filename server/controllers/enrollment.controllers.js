...
const find =async(req,res,next)=>{
    try {
        const enrollment = await Enrollment.findOne({student:req.auth._id,course:req.course._id})
        if(!enrollment){
            return next()
        }else{
...
const enrollmentById = async(req,res,next,id)=>{
    try {
        const enrollment = await Enrollment.findById(id).populate({path:'course',populate:{path:'instructor'}}).populate('student','_id first_name last_name')
...
const complete = async(req,res)=>{
    try {
        ...
        await Enrollment.updateOne({'lessonStatus._id':req.body.lessonStatusId},{'$set':updatedEnrollment})
        return res.status(200).json({
            message: 'Updated!'
        })
    } catch (err) {
